...
def zoo_gradient(data, model, label): 
    '''
        This function is used for estimating gradient rather than using .backward
        Input:
            data: batch data, format: (-1, 1, 28, 28)
            model: the model trained with logits rather than 0, 1 label,
                    this model is used for generating logits and the logits is used to compute loss
            label: the original 0, 1 label
        Output:
            grads: estimated gradient (np.array) format: (-1, 1, 28, 28)
    '''
    grads = []
    for i in range(len(data)):
        tmp_data = data[i]
        data_len = torch.numel(tmp_data)
        # for each data, copy data_len*2+1 times and each pixel is changed for two times
        var_data = tmp_data.repeat(data_len * 2 + 1, 1, 1 ,1)
        j = 0
 
        # each pixel is changed for two times and compute the corresponding losses
        for m in range(tmp_data.shape[1]):
            for n in range(tmp_data.shape[2]):
                var_data[j*2+1][0, m, n]+= 0.1
                var_data[j*2+2][0, m, n] -= 0.1
                j += 1

        grad = torch.zeros(tmp_data.size(), dtype = torch.float32)
        output = model(var_data)

        losses = torch.zeros(len(output))
        for k in range(len(output)): # compute the losses with respect to each pixel's change
            losses[k] = F.cross_entropy(output[k].unsqueeze(0), label[i].unsqueeze(0))
        
        j = 0
        for m in range(tmp_data.shape[1]): # compute the estimated gradient corresponds to each pixel
            for n in range(tmp_data.shape[2]):
                grad[0, m, n] = (losses[j*2 + 1] - losses[j*2 + 2]) / 0.2
                j += 1
        grads.append(np.array(grad.data, dtype = np.float32))
...
class Meta(nn.Module):
    ...
    def forward(self, batch_data, device):
        ...
        picked_points = np.random.choice(784, random_point_number)
    ...
    def finetunning(self, test_data,model,device,step_size=0.3):
        ...
        :param y_qry:   [querysz]
        :param test_data: [x_spt, y_spt, label_spt,x_qry, y_qry,label_qry] spt.shape = batch_size,k_shot,c_,h,w
        :return:
        """
