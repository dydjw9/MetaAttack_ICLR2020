...
class generate_gradient:
    ...
    def __init__(self, device, targeted = False, classes = 200, debug = False):
        self.debug = debug
    ...
    def run(self, model, img, target, indice):
        ...
        half_po = img_var.size()[0]/2
        half_po = int(half_po)
        output_0 = F.softmax(model(img_var[:half_po]), dim = 1)
        output_1 = F.softmax(model(img_var[half_po:]), dim = 1)
        output = torch.cat([output_0,output_1],0)
